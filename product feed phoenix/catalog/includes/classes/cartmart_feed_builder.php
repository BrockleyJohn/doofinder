...
class cartmart_feed_builder {

  protected $categories_flat;
  protected $categories_parents;
  ...
  public static function gtin_valid_check($code) {
  ...
  public function __construct($language_code, $currencies, $file_stub) {
    if (! array_key_exists($language_code, $GLOBALS['languages'])) {
      throw new Exception('Language ' . $language_code . ' not enabled for site');
    }
    $this->language_code = $language_code;
    $this->language_id = $GLOBALS['languages'][$language_code]['id'];
    $this->currencies = $currencies;
    $this->file_stub = $file_stub;
    $this->build_categories_tree();
    $this->build_product_categories();
    $this->build_product_types();
    $this->build_product_images();
    $this->build_product_data();
    $this->pub_date = date(DATE_ISO8601);
  }

  public function get_filename($currency) {
    if (! in_array($currency, $this->currencies)) {
      throw new Exception('Currency ' . $currency . ' not enabled for feed');
    }
    return $this->file_stub . '_' . $this->language_code . '_' . $currency . '.xml';
  }

  public function get_xml($currency) {
    if (! in_array($currency, $this->currencies)) {
      throw new Exception('Currency ' . $currency . ' not enabled for feed');
    }
    $this->xml = new google_xml_builder();
    $this->xml->Push("rss", array("version" => "2.0",
                                  "xmlns:g" => "http://base.google.com/ns/1.0"));
    $this->xml->Push("channel");

    $this->add_feed_info($currency);
    $this->add_items($currency);
  ...
  protected function add_feed_info($currency) {
    $this->xml->Element("title", STORE_NAME);
    $this->xml->Element("link", HTTP_SERVER);
    $this->xml->Element("description", STORE_NAME . ' ' . $GLOBALS['languages'][$this->language_code]['name'] . ' ' . $currency);
    $this->xml->Element("pubDate", $this->pub_date);
  }

  protected function add_items($currency) {
    foreach ($this->products as $product) {
      $this->add_item($product, $currency);
    }
  }

  protected function add_item($product, $currency) {
    $this->xml->Push('item');

    // Required, global namespace.
    $this->add_title($product); // add seo_title or products_name
    $this->add_link($product); // product url
    $this->add_description($product); // add seo_description or products_description after cleaning and stripping html
    ...
    $this->add_id($product);
    $this->add_price($product, $currency);
    $this->add_condition($product);  
    $this->add_mpn($product);  
    $this->add_gtin($product); 

    // Optional, Google namespace.
    $this->add_sale_price($product, $currency); // if has a special, add sale price and sale effective dates
    $this->add_image_links($product); // both the main image and any additional images (includes thumb check)
    $this->add_weight($product);
    $this->add_model_number($product);
    $this->add_payment_notes($product);
    $this->add_product_type($product);
    $this->add_quantity($product);  // add availability as well as quantity

    $this->xml->Pop('item');
  }

  protected function add_title($product) {
    $title = Text::is_empty($product['products_seo_title']) ? $product['products_name'] : $product['products_seo_title'];
    // things like &reg; break google feed
    $title = $this->named_entities_to_numeric($title);
    $this->add_if_not_empty('title', $title);
  }

  protected function add_link($product) {
    $this->add_if_not_empty('link', $GLOBALS['Linker']->build('product_info', ['products_id' => $product['products_id']], false));
  }
  
  protected function add_description($product) {
    $description = Text::is_empty($product['products_seo_description']) ? $product['products_description'] : $product['products_seo_description'];
    // put spaces after some tags before stripping them - stops words running together
    ...
    // things like &reg; break google feed
    $description = $this->named_entities_to_numeric($description);
    // strip invalid characters
    $description = iconv('UTF-8', 'UTF-8//IGNORE', $description);
    // enforce 5000 character limit
    if (strlen($description) > 5000) {
      // truncate to last space before 5000 characters
      $longlines = wordwrap($description, 4997, "\n");
      $description = array_shift(explode("\n", $longlines)) . '...';
    }
    $this->add_if_not_empty('description', $description);
  }

  protected function add_pubdate() {
    $this->add_if_not_empty('pubDate', $this->pub_date);
  }

  protected function add_brand($product) {
    $this->add_if_not_empty('g:brand', $product['manufacturers_name'] ?? '');
  }

  protected function add_id($product) {
    $id = $product['products_id'];
    $this->add_if_not_empty('g:id', $id);
  }

  protected function add_price($product, $currency) {
    $price = Currencies::round($GLOBALS['currencies']->calculate_price(($product['products_price'] * $GLOBALS['currencies']->currencies[$currency]['value']), Tax::get_rate($product['products_tax_class_id'])), $GLOBALS['currencies']->currencies[$currency]['decimal_places']);
    $this->add_if_not_empty('g:price', $price . ' ' . $currency);
  }

  protected function add_condition($product) {
    $this->add_if_not_empty('g:condition', 'new');
  }

  protected function add_mpn($product) {
    $this->add_if_not_empty('g:mpn', $product['products_model']);
  }

  protected function add_gtin($product) {
    if (! Text::is_empty($product['products_gtin']) && self::gtin_valid_check($product['products_gtin'])) {
      $this->add_if_not_empty('g:gtin', $product['products_gtin']);
    }
  }

  protected function add_sale_price($product, $currency) {
    if ($product['products_price'] != $product['final_price']) {
      $price = Currencies::round($GLOBALS['currencies']->calculate_price(($product['specials_new_products_price'] * $GLOBALS['currencies']->currencies[$currency]['value']), Tax::get_rate($product['products_tax_class_id'])), $GLOBALS['currencies']->currencies[$currency]['decimal_places']);
      $this->add_if_not_empty('g:sale_price', $price . ' ' . $currency);
      $sale_start = date(DATE_ISO8601, strtotime($product['specials_date_added']));
      if ((! Text::is_empty($product['specials_expires_date'])) && strtotime($product['specials_expires_date']) > time()){
        $sale_end = date(DATE_ISO8601, strtotime($product['specials_expires_date']));
      } else {
        $sale_end = date(DATE_ISO8601, strtotime("+31 day")); // a day after the product feed expires
      }
      $this->add_if_not_empty('g:sale_price_effective_date', $sale_start . '/' . $sale_end);
    }
  }

  protected function add_image_links($product) {
		$done = false; $i = 1;
		$thumb = pathinfo($product['products_image']); // this is(was) usually a thumbnail image so check if there's a bigger version
    // cater for modular admin categories addon (thumbs prefixed th_)
		$stub = (strpos($thumb['filename'],'th_') === 0 ? substr($thumb['filename'],3) : false); 
		$th_main = ($stub ? $thumb['dirname'] . DIRECTORY_SEPARATOR . $stub . '.' . $thumb['extension'] : '');
    // cater for additional images addon (sized images suffixed with s, m and l)
		$stub = (substr($thumb['filename'], -1) == 's' ? substr($thumb['filename'],0,-1) : false); 
		$sl_main = ($stub ? $thumb['dirname'] . DIRECTORY_SEPARATOR . $stub . 'l.' . $thumb['extension'] : '');

    if (array_key_exists($product['products_id'], $this->product_images) && is_array($this->product_images[$product['products_id']])) {
      foreach ($this->product_images[$product['products_id']] as $image) {
        $image_link = $GLOBALS['Linker']->build('images/' . strtr($image, [' ' => '%20']), [], false);
        if ($done == false && $image == $th_main) {
          $this->add_if_not_empty('g:image_link', $image_link);
          $done = true;
        } elseif ($done == false && $image == $sl_main) {
          $this->add_if_not_empty('g:image_link', $image_link);
          $done = true;
        } elseif ($i < 10) {  // now a limit of 10 additional images
    ...
    if ($done == false) {
      $this->add_if_not_empty('g:image_link', $GLOBALS['Linker']->build('images/' . strtr($product['products_image'], [' ' => '%20']), [], false));
    }
  }

  protected function add_weight($product) {
    $this->add_if_not_empty('g:product_weight', $product['products_weight'] . ' ' . cartmart_product_feed::get_weight_unit());
  }

  protected function add_model_number($product) {
    $this->add_if_not_empty('g:model_number', $product['products_model']);
  }

  protected function add_payment_notes($product) {
    // think this is abandoned
  }

  protected function add_product_type($product) {
    foreach ($this->product_categories[$product['products_id']] as $category) {
      $this->add_if_not_empty('g:product_type', $this->product_types[$category] ?? '');
    }
  }

  protected function add_quantity($product) {
    $this->add_if_not_empty('g:quantity', $product['products_quantity']);
    if ($product['products_quantity'] > 0) {
      $availability = 'in stock';
    } elseif ((! Text::is_empty($product['products_date_available'])) && strtotime($product['products_date_available']) > time() && strtotime($product['products_date_available']) < strtotime("+1 year")) {
      $availability = 'preorder';
      $this->add_if_not_empty('g:availability_date', date(DATE_ISO8601, strtotime($product['products_date_available'])));
    } else {
  ...
  protected function add_if_not_empty($element, $content) {
  ...
  protected function build_categories_tree() {
    ...
    while ( $category = $categories_query->fetch_assoc() ) {
      $this->categories_flat[$category['categories_id']] = $category['categories_name'];
      $this->categories_parents[$category['categories_id']] = $category['parent_id'];
    }
    
  }

  protected function build_product_categories() {
    $prods_cats_query = $GLOBALS['db']->query('SELECT categories_id, p2c.products_id FROM products_to_categories p2c INNER JOIN products p ON p2c.products_id = p.products_id WHERE p.products_status = 1');
    $products_id = 0;
    while ($prod_cat = $prods_cats_query->fetch_assoc()) {
      if ($products_id != $prod_cat['products_id']) {
        $products_id = $prod_cat['products_id'];
        $this->product_categories[$products_id] = [];
      }
      $this->product_categories[$products_id][] = $prod_cat['categories_id'];
    }
  }

  protected function build_product_data() {
    $products_query = $GLOBALS['db']->query('SELECT p.products_id, p.products_model, pd.products_name, pd.products_seo_title, pd.products_description, pd.products_seo_description, p.products_image, p.products_price, p.products_tax_class_id, IF(s.status, s.specials_new_products_price, NULL) AS specials_new_products_price, IF(s.status, s.specials_date_added, NULL) as specials_date_added, IF(s.status, s.expires_date, NULL) AS specials_expires_date, IF(s.status, s.specials_new_products_price, p.products_price) AS final_price, p.products_gtin, p.products_quantity, p.products_weight, p.products_date_added, p.products_last_modified, p.products_date_available FROM products_description pd INNER JOIN products p ON p.products_id = pd.products_id LEFT JOIN manufacturers m ON m.manufacturers_id = p.manufacturers_id LEFT JOIN specials s ON s.products_id = p.products_id WHERE pd.language_id = ' . $this->language_id . ' AND p.products_status = 1');
    while ($product = $products_query->fetch_assoc()) {
      $this->products[] = $product;
    }
  }

  protected function build_product_images() {
    $this->products_images = [];
    $prods_imgs_query = $GLOBALS['db']->query('SELECT p.products_id, image FROM products_images ai INNER JOIN products p ON ai.products_id = p.products_id WHERE p.products_status = 1');
    $products_id = 0;
    while ($prod_img = $prods_imgs_query->fetch_assoc()) {
      if ($products_id != $prod_img['products_id']) {
        $products_id = $prod_img['products_id'];
        $this->product_images[$products_id] = [];
      }
      $this->product_images[$prod_img['products_id']] = $prod_img['image'];
    }
  }

  protected function build_product_types() {
    $filled_cat_query = $GLOBALS['db']->query('SELECT c.categories_id FROM categories c INNER JOIN products_to_categories p2c ON p2c.categories_id = c.categories_id INNER JOIN products p ON p.products_id = p2c.products_id WHERE p.products_status = 1');
    while ($filled_cat = $filled_cat_query->fetch_assoc()) {
      $this->product_types[$filled_cat['categories_id']] = $this->product_type_create($filled_cat['categories_id']);
    }
  }

  protected function named_entities_to_numeric($string) {
    $HTML401NamedToNumeric = [
  ...
  protected function product_type_create($category_id) {
    $cat_array = [];
    $this->walk_up_tree($category_id, $cat_array);
    $cat_array = array_reverse($cat_array);
    $product_type = '';
    foreach ($cat_array as $cat_name) {
      $product_type .= $cat_name . ' > ';
    }
    return substr($product_type, 0, -3);
  }

  protected function walk_up_tree($category_id, &$cat_array) {
    $name = $this->categories_flat[$category_id];
    array_push($cat_array, $name);
    $parent_id = $this->categories_parents[$category_id];
    if ($parent_id > 0) {
      $this->walk_up_tree($parent_id, $cat_array);
    }
  }
}
